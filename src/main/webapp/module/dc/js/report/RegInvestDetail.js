...
dc.report.regInvestDetail=function(winId,winName,desk) {
	var resultPanelId='dc_regInvestDetailP';
	var curDate = new Date();
	var date=Ext.Date.format(curDate,'Y/m/d');
	var bdate;
	var edate;
	function getHtml(begandate,enddate) {
		var contextHtml = '<iframe src="'+constant.page+'dc/regInvestDetail?BATCH_SQL=reportcheck_queryregreginvestdetail,reportcheck_queryreginvestotal&begandate='+begandate+'&enddate='+enddate+'" width="100%" height="100%" frameborder="0" scrolling="auto"></iframe>';
		return contextHtml;
	}
	var resultPanel=new Ext.Panel({
		tbar:[
			...
			{icon:constant.iconPath+'excel.png',tooltip:'下载Excel',handler:function(){
				...
				document.getElementById('main_hiddenFrame').src=constant.page+'dc/regInvestDetail?VIEW=excel&BATCH_SQL=reportcheck_queryregreginvestdetail,reportcheck_queryreginvestotal&begandate='+begandate+'&enddate='+enddate;
			}}
		],
		id:resultPanelId,
		region:'center',
