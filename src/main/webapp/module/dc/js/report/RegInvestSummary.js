...
dc.report.regInvestSummary=function(winId,winName,desk) {
	var resultPanelId='dc_regInvestSummaryP';
	var curDate = new Date();
	var date=Ext.Date.format(curDate,'Y/m/d');
	var bdate;
	var edate;
	function getHtml(begandate,enddate) {
		var contextHtml = '<iframe src="'+constant.page+'dc/regInvestSummary?BATCH_SQL=reportcheck_queryreginvestsummary&begandate='+begandate+'&enddate='+enddate+'" width="100%" height="100%" frameborder="0" scrolling="auto"></iframe>';
		return contextHtml;
	}
	var resultPanel=new Ext.Panel({
		tbar:[
			...
			{icon:constant.iconPath+'excel.png',tooltip:'下载Excel',handler:function(){
				...
				document.getElementById('main_hiddenFrame').src=constant.page+'dc/regInvestSummary?VIEW=excel&BATCH_SQL=reportcheck_queryreginvestsummary&begandate='+begandate+'&enddate='+enddate;
			}}
		],
		id:resultPanelId,
		region:'center',
