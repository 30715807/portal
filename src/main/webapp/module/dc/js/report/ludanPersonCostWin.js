...
dc.report.ludanPersonCostWin=function(winId,winName,desk) {
	var resultPanelId='dc_ludanPersonCostP';
	var curDate = new Date();
	var date=Ext.Date.format(curDate,'Y/m/d');
	var bdate;
	var edate;
	function getHtml(begandate,enddate) {
		var contextHtml = '<iframe src="'+constant.page+'dc/ludanPersonCost?BATCH_SQL=reportcheck_brhldrenyuancostlistexcel,reportcheck_brhludanpersoncount&begandate='+begandate+'&enddate='+enddate+'" width="100%" height="100%" frameborder="0" scrolling="auto"></iframe>';
		return contextHtml;
	}
	var resultPanel=new Ext.Panel({
		tbar:[
			bdate=Ext.create({xtype:'datefield',fieldLabel:'进件开始日期',format: 'Y/m/d',value:Ext.util.Format.date(new Date(),"Y/m/d"),labelWidth:90,width:220}),
			edate=Ext.create({xtype:'datefield',fieldLabel:'进件结束日期',format: 'Y/m/d',value:Ext.util.Format.date(new Date(),"Y/m/d"),labelWidth:90,width:220}),
			'-',
			...
			{icon:constant.iconPath+'excel.png',tooltip:'下载Excel',handler:function(){
				...
				document.getElementById('main_hiddenFrame').src=constant.page+'dc/ludanPersonCost?VIEW=excel&BATCH_SQL=reportcheck_brhldrenyuancostlistexcel,reportcheck_brhludanpersoncount&begandate='+begandate+'&enddate='+enddate;
			}}
		],
		id:resultPanelId,
		region:'center',
